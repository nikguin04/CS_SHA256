...
namespace Sha256Prog
{
    ...
    {
        private static string[] hashValues;
        private static string[] roundConstants;
        public static void Init() // Kørt hver gang programmet starter
        {
            hashValues = ConstantGenerator.generateConstants(8, RootMode.Square);
            roundConstants = ConstantGenerator.generateConstants(64, RootMode.Cube);
        ...
        public static string SHA256Encrypt(string input)
        {
            ...
            {
                //Console.WriteLine(roundConstants[i].Length);
                bitRoundConstants.SetValue(HexToBits(roundConstants[i]), i);
                //Console.WriteLine("index " + i + " bin: " + BitsToString(bitHashValues[i]));
            }


            Byte[] bytes = Encoding.UTF8.GetBytes(input); // omdanner vores input til bytes, ud fra ASCII tabellen
            long bitlength = bytes.Length * 8; // længde af vores input it bits
            bytes = bytes.Append((byte)0x80).ToArray(); // append 1 (and 7x 0), byte 0x80 = bit 10000000
            while ((bytes.Length + (64 / 8)) % (512 / 8) > 0)
            {
                bytes = bytes.Append((byte)0x00).ToArray(); // byte 0x00 = bit 00000000 
            }

            Byte[] byteLength = BitConverter.GetBytes(bitlength); // får længden af vores input
            for (int i = 0; i < (64 / 8); i++) // tilføjer de sidste 64 bits (8 bytes) som beskriver længden
            {
            ...
            for (int chunkIndex = 0; chunkIndex < bytes.Length / 64; chunkIndex++)
            {
                ...
                BitArray blankbit32 = new BitArray(32, false);
                ...
                bitHashValues = new BitArray[hashValues.Length]; // hashValues.Length == 8
                for (int i = 0; i < hashValues.Length; i++) // for loopet kører 8 gange
                {
        ...
        static BitArray AddBits(BitArray arr1, BitArray arr2) // if number overflows, remove last 1
        {
            try
            {
                ...
                var overlap = false;
                //arr1 = new BitArray(arr1);
                var addArray = new BitArray(arr1.Length, false);

                for (int i = 0; i < arr1.Length; i++)
                {
                    int index = arr1.Length - i - 1;
                    bool currentbit = false;
                    if (arr1[index] || arr2[index] || overlap)
                    {
                        int total = (arr1[index] ? 1 : 0) + (arr2[index] ? 1 : 0) + (overlap ? 1 : 0);
                        if (total == 1)
                        {
                            currentbit = true;
                            overlap = false;
                    ...
                    addArray.Set(index, currentbit);
                }

                return addArray;
            } catch (Exception e)
            {
                Console.WriteLine("AddBits Error: " + e.Message);
